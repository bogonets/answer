...
export function install_LGraphCanvas(vm, litegraph) {
  var vm = vm;
  var LiteGraph = litegraph;

  function LGraphCanvas(canvasElement, graph, options) {
    options = options || {};
    ...
    this.background_image =
      "data:image/*;base64,";
    //custom2
    // this.background_image = 'data:image/*;base64,';

    if (canvasElement && canvasElement.constructor === String)
    ...
    this.link_type_colors = { "-1": "#F85", number: "#AAA", node: "#DCA" };
    this.gradients = {}; //cache of gradients
  }

  vm.LGraphCanvas = LiteGraph.LGraphCanvas = LGraphCanvas;
  ...
  LGraphCanvas.prototype.processMouseDown = function(e) {
    ...
    if (node) {
      node.before_pos = LiteGraph.cloneObject(node.pos);
    ...
    if (e.which == 1) {
      ...
      if (node && this.allow_interaction && !skip_action) {
        if (!this.live_mode && !node.flags.pinned) this.bringToFront(node); //if it wasnt selected?
        ...
        if (!this.connecting_node && !node.flags.collapsed && !this.live_mode) {
          ...
          if (
            ...
            LiteGraph.isInsideRectangle(
              e.canvasX,
              e.canvasY,
              node.pos[0] + node.size[0] - LiteGraph.RESIZE_AREA_SIZE,
              node.pos[1] + node.size[1] - LiteGraph.RESIZE_AREA_SIZE,
              LiteGraph.RESIZE_AREA_SIZE,
              LiteGraph.RESIZE_AREA_SIZE
            )
          ) {
            ...
            if (node.inputs)
              for (var i = 0, l = node.inputs.length; i < l; ++i) {
                ...
                if (
                  ...
                  if (input.link !== null || input.link !== -1) {
        ...
        if (this.selected_group) {
          ...
          this.selected_group.onSelected();
        }
        this.selected_group_resizing = false;
        if (this.selected_group) {
          if (e.ctrlKey) this.dragging_rectangle = null;

          var dist = LiteGraph.distance(
            [e.canvasX, e.canvasY],
            [
          ...
          if (dist * this.scale < 10) this.selected_group_resizing = true;
        ...
        /* unused searchdialog.
                // if (is_double_click) // unuse double click.
                // this.showSearchBox(e); // searchDialog open.
                */
  ...
  LGraphCanvas.prototype.processMouseMove = function(e) {
    ...
    if (this.dragging_rectangle) {
      ...
      if (this.selected_group_resizing)
        ...
        this.selected_group.preventResizeForMinSize(
      ...
      if (node) {
        ...
        this._highlight_input = null; // Yellow linked slot erage without slot.
      }

      //get task over
      var task = this.graph.getGroupOnPos(e.canvasX, e.canvasY);
      if (task) {
        for (var i = 0, l = this.graph._tasks.length; i < l; ++i) {
          if (task != this.graph._tasks[i]) {
            //other task Mouse leave.
            if (this.graph._tasks[i].onMouseLeave)
              this.graph._tasks[i].onMouseLeave();
          }
        }
        // task mouse over.
        if (task.onMouseEnter) {
          task.onMouseEnter();
        }
      } else {
        for (var i = 0, l = this.graph._tasks.length; i < l; ++i) {
      ...
      if (this.resizing_node && !this.live_mode) {
        ...
        var myTask = this.resizing_node.getJoinedTask();
        if (
          this.resizing_node.pos[0] + this.resizing_node.size[0] >
          myTask.pos[0] + myTask.size[0]
        ) {
          var gap =
            this.resizing_node.pos[0] +
            this.resizing_node.size[0] -
            (myTask.pos[0] + myTask.size[0]);
          this.resizing_node.size[0] -= gap + 5;
        }

        if (
          this.resizing_node.pos[1] + this.resizing_node.size[1] >
          myTask.pos[1] + myTask.size[1]
        ) {
          var gap =
            this.resizing_node.pos[1] +
            this.resizing_node.size[1] -
            (myTask.pos[1] + myTask.size[1]);
          this.resizing_node.size[1] -= gap + 5;
        }
        ...
        var min_height =
          ...
          (this.resizing_node.widgets ? this.resizing_node.widgets.length : 0) *
  ...
  LGraphCanvas.prototype.processMouseUp = function(e) {
    ...
    if (e.which == 1) {
      ...
      for (var task in this.graph._tasks) {
        for (var node in this.selected_nodes) {
          // console.log("group", group, "node", node);
          this.graph._tasks[task].recomputeInsideNodes2(
            Number(task),
      ...
      // console.log("Select Nodes:", this.selected_nodes["2"]);
      // console.log("Total Groups:", this.graph._groups);
      this.selected_group_resizing = false;
      ...
      if (this.dragging_rectangle) {
        ...
        if (this.resizing_node.onResizeStop) {
          this.resizing_node.onResizeStop();
        }
        this.dirty_canvas = true;
        this.dirty_bgcanvas = true;
  ...
  LGraphCanvas.prototype.isOverNodeInput = function(
    ...
    if (node.inputs)
      for (var i = 0, l = node.inputs.length; i < l; ++i) {
        var input = node.inputs[i];
        var link_pos = node.getConnectionPos(true, i);
        // if (LiteGraph.isInsideRectangle(canvasx, canvasy, link_pos[0] - 10, link_pos[1] - 5, 20, 10)) {
        //                                    x,       y,             left,             top,  w,  h
        if (
          LiteGraph.isInsideRectangle(
            canvasx,
    ...
    if (e.type == "keydown") {
      ...
      if (e.keyCode == 46) {
        if (e.target.localName != "input" && e.target.localName != "textarea") {
          var selected_tasks = [];
          for (var i in this.graph._tasks) {
            if (this.graph._tasks[i].selected) {
              selected_tasks.push(this.graph._tasks[i]);
            }
          }
    ...
    for (var i = 0; i < selected_nodes_array.length; ++i) {
      ...
      if (node.inputs && node.inputs.length)
        for (var j = 0; j < node.inputs.length; ++j) {
          ...
          if (!input || input.link == null || input.link == -1) continue;
      ...
      if (files && files.length) {
        for (var i = 0; i < files.length; i++) {
          ...
          if (node.onDropData) {
            ...
            if (type == "text" || type == "") reader.readAsText(file);
  ...
  LGraphCanvas.prototype.selectNodes = function(
    ...
    for (var i = 0; i < nodes.length; ++i) {
      ...
      if (!node.selected && node.onSelected) this.deselectAllTasks();
      node.onSelected();
      node.selected = true;
  ...
  /**
   * removes all tasks from the current selection
   * @method deselectAllTasks
   **/
  LGraphCanvas.prototype.deselectAllTasks = function() {
    if (!this.graph) return;
    var tasks = this.graph._tasks;
    for (var i = 0, l = tasks.length; i < l; ++i) {
      var task = tasks[i];
      if (!task.selected) continue;
      if (task.onDeselected) task.onDeselected();
    }
    this.setDirty(true);
  };
  ...
  LGraphCanvas.prototype.deleteSelectedNodes = function() {
    for (var i in this.selected_nodes) {
      ...
      // this.graph.remove(m);
      LGraphCanvas.onMenuNodeRemove(null, null, null, null, m);
    }
    this.selected_nodes = {};
    this.highlighted_links = {};
    this.setDirty(true);
  ...
  LGraphCanvas.prototype.centerOnNode = function(node) {
    this.offset[0] =
      -node.pos[0] -
      node.size[0] * 0.5 +
      (this.canvas.width * 0.5) / this.scale;
    this.offset[1] =
      -node.pos[1] -
      node.size[1] * 0.5 +
      (this.canvas.height * 0.5) / this.scale;
    this.setDirty(true, true);
  };
  ...
  LGraphCanvas.prototype.adjustMouseEvent = function(e) {
    ...
    // e.deltaX = e.localX - this.last_mouse_position[0];  // wrong code. deltaX is readonly value.
    // e.deltaY = e.localY - this.last_mouse_position[1];  // wrong code. deltaY is readonly value.
  ...
  LGraphCanvas.prototype.convertEventToCanvas = function(e) {
    ...
    return this.convertOffsetToCanvas([
      e.pageX - rect.left,
      e.pageY - rect.top,
    ]);
  };
  ...
  LGraphCanvas.prototype.drawFrontCanvas = function() {
    ...
    if (this.graph) {
      ...
      if (this.connecting_pos != null) {
        ...
        this.renderLink(
          ...
          this.connecting_output.dir ||
            (this.connecting_node.flags.horizontal
              ? LiteGraph.DOWN
              : LiteGraph.RIGHT),
          LiteGraph.CENTER
        );
      ...
      if (this.onDrawForeground) this.onDrawForeground(ctx, this.visible_rect);
  ...
  LGraphCanvas.prototype.drawBackCanvas = function() {
    ...
    if (this.graph) {
      ...
      if (this.background_image && !bg_already_painted) {
        // this.scale > 0.5 -> [Background image visible limited value.]
        // if (this.background_image && this.scale > 0.5 && !bg_already_painted) {
        // [zoomin zoomout -> blur the image.]
      ...
      //groups
      if (this.graph._tasks.length && !this.live_mode)
        this.drawTasks(canvas, ctx);
  ...
  LGraphCanvas.prototype.drawNode = function(node, ctx) {
    ...
    if (!node.flags.collapsed) {
      ...
      if (node.widgets) {
        if (node.flags.horizontal || node.flags.widgets_up) max_y = 2;
      ...
      if (node.inputs) {
        for (var i = 0; i < node.inputs.length; i++) {
          var slot = node.inputs[i];
          if (slot.link == null || slot.link == -1) continue;
          input_slot = slot;
          break;
        }
      }
      ...
      if (input_slot) {
        ...
        if (
          ...
          ctx.rect(
            x - 7 + 0.5,
            y + 4 - LiteGraph.NODE_TITLE_HEIGHT * 0.5 + 0.5,
            14,
            LiteGraph.NODE_TITLE_HEIGHT - 8
          );
        } else if (slot.shape === LiteGraph.ARROW_SHAPE) {
          ctx.moveTo(x + 8, y);
          ctx.lineTo(x + -4, y - 4);
  ...
  LGraphCanvas.prototype.drawNodeShape = function(
    ...
    if (!node.flags.collapsed) {
      ...
      if (shape == LiteGraph.BOX_SHAPE || this.scale < 0.5) {
        ...
        ctx.moveTo(area[0] + area[2] - 7, area[1] + area[3]);
        ctx.lineTo(area[0] + area[2], area[1] + area[3] - 7);
        ctx.moveTo(area[0] + area[2] - 4, area[1] + area[3]);
        ctx.lineTo(area[0] + area[2], area[1] + area[3] - 4);
      } else if (
    ...
    if (node.bgImage && node.bgImage.width)
      ctx.drawImage(
        node.bgImage,
        (size[0] - node.bgImage.width) * 0.5,
        (size[1] - node.bgImage.height) * 0.5
      );

    if (node.bgImageUrl && !node.bgImage)
      node.bgImage = node.loadImage(node.bgImageUrl);

    if (node.onDrawBackground) node.onDrawBackground(ctx, this, this.canvas);
    ...
    if (render_title || title_mode == LiteGraph.TRANSPARENT_TITLE) {
      ...
      if (title_mode != LiteGraph.TRANSPARENT_TITLE) {
        ...
        if (this.use_gradients) {
          var grad = this.gradients[fgcolor];
          if (!grad) {
            grad = this.gradients[fgcolor] = ctx.createLinearGradient(
              0,
              0,
              400,
              0
      ...
      if (this.scale > 0.5) {
        ...
        if (title) {
          if (selected) ctx.fillStyle = node.title_color || "white";
          else
            ctx.fillStyle =
              node.title_color ||
          ...
          if (node.flags.collapsed) {
            ...
            ctx.textAlign = "left";
            ctx.fillText(title, 5, -title_height * 0.3);
            // ctx.textAlign = "left";
          } else {
            ctx.textAlign = "left";
  ...
  LGraphCanvas.prototype.drawConnections = function(ctx) {
    ...
    var margin_area = new Float32Array([
      visible_area[0] - 20,
      visible_area[1] - 20,
      visible_area[2] + 40,
      visible_area[3] + 40,
    ...
    for (var n = 0, l = nodes.length; n < l; ++n) {
      ...
      for (var i = 0; i < node.inputs.length; ++i) {
        ...
        var start_dir =
          start_slot.dir ||
          (start_node.flags.horizontal ? LiteGraph.DOWN : LiteGraph.RIGHT);
        var end_dir =
          end_slot.dir ||
          (node.flags.horizontal ? LiteGraph.UP : LiteGraph.LEFT);
      ...
      switch (w.type) {
        ...
        case "combo":
          ...
          if (show_text) {
            ...
            ctx.fill();
            if (
              (w.value == "" || w.value == null || w.value == undefined) &&
              w.name != null
            ) {
              ctx.fillStyle = "#999";
        ...
        case "text":
          ...
          if (show_text) {
            ...
            if (
              ...
              // console.log("VALUE:", w.value, typeof w.value);
              ctx.fillText(w.name, 20, y + H * 0.7);
            }
            ctx.fillStyle = "#DDD";
            ctx.textAlign = "right";
            ctx.fillText(w.value, width - 20, y + H * 0.7);
            // console.log(ctx.measureText(w.value).width, width);
            if (ctx.measureText(w.value).width > width) {
              node.size[0] = ctx.measureText(w.value).width;
              this.drawNodeWidgets(node, posY, ctx);
            }
          }
          break;
        default:
      ...
      if (
        ...
        switch (w.type) {
          ...
          case "slider":
            ...
            if (w.callback)
              setTimeout(function() {
                w.callback(w.value, that, node, pos);
              }, 20);
            this.dirty_canvas = true;
            break;
          case "number":
          case "combo":
            ...
            if (w.callback)
              setTimeout(
                function() {
                  this.callback(this.value, that, node, pos);
                }.bind(w),
                20
              );
            this.dirty_canvas = true;
          ...
          case "text":
            if (event.type == "mousedown")
              this.prompt(
                ...
                function(v) {
                  this.value = v;
                  if (w.callback) w.callback(v, that, node);
                }.bind(w),
                event
              );
            break;
   ...
   * @method drawTasks
   **/
  LGraphCanvas.prototype.drawTasks = function(canvas, ctx) {
    if (!this.graph) return;
    ...
    for (var i = 0; i < tasks.length; ++i) {
      ...
      if (!LiteGraph.overlapBounding(this.visible_area, task._bounding))
        continue; //out of the visible area

      ctx.fillStyle = task.color || "#335";
      ctx.strokeStyle = task.color || "#335";
      var pos = task._pos;
      var size = task._size;
      ctx.globalAlpha = 0.6 * this.editor_alpha;
      ctx.beginPath();
      ...
      var font_size = task.font_size || LiteGraph.DEFAULT_GROUP_FONT_SIZE;
      ctx.font = font_size + "px Arial";
      ctx.fillText(task.title, pos[0] + 4, pos[1] + font_size);
      if (!vm.$vuetify.theme.dark) {
        ctx.strokeStyle = "black";
        ctx.strokeText(task.title, pos[0] + 4, pos[1] + font_size);
      }
    }
  ...
  LGraphCanvas.onTaskAdd = function(info, entry, mouse_event) {
    var canvas = LGraphCanvas.active_canvas;
    var ref_window = canvas.getCanvasWindow();

    var task = new LiteGraph.LGraphTask("Task");
    task.id = ++canvas.graph.last_task_id;
    task.size = [200, 150];
    task.pos = canvas.convertEventToCanvas(mouse_event);
    task.title = LiteGraph.makeTaskTitle(task, canvas.graph._tasks);

    // All deselect and Selected Task when create task.
    canvas.deselectAllNodes();
    canvas.deselectAllTasks();
    task.onSelected();

    canvas.graph.add(task);

    task.computeInputSocket();
  };

  LGraphCanvas.onTaskClone = function(vm, event) {
    let canvas = LGraphCanvas.active_canvas;
    let ref_window = canvas.getCanvasWindow();

    let task = new LiteGraph.LGraphTask("Task");

    task.id = ++canvas.graph.last_task_id;
    task.size = vm.task.size;
    task.nodes = [];

    // clone nodes
    for (const _node of vm.task._nodes) {
      let node = LiteGraph.createNode(_node.type);
      node.id = ++canvas.graph.last_node_id;
      node.title = LiteGraph.makeLambdaTitle(node, canvas.graph._nodes, task);
      node.pos = reposition(_node.pos, vm.task.pos);
      node.task_id = task.id;
      node.properties = _node.properties;
      node.uid = `${node.type}${node.id}`;

      task.taskComputeSize(node);
      task._nodes.push(node);
      task.nodes.push(node);

      // vm.task.graph.last_link_id;
      // vm.task.graph.links;

      // const length = Object.values(vm.task.graph.links).length;
      // let linkObjs = {};
      // let linkId = length +2;
      // // for(let i=2; i < length; i++) {
      // for(var link of Object.values(vm.task.graph.links)){
      //     if(link.origin_id === _node.id){
      //         var linkObj = {};
      //         linkObj[linkId] = link;
      //         linkObj[linkId].origin_id = node.id;
      //         Object.assign(linkObjs, linkObj);
      //     }
      // }
    }
  ...
  function reposition(position, diff) {
    var x = position[0] - diff[0];
    var y = position[1] - diff[1];
  ...
  LGraphCanvas.onMenuAdd = function(node, options, e, prev_menu) {
    ...
    function inner_create(v, e) {
      ...
      if (node) {
        ...
        var task = canvas.graph.getGroupOnPos(node.pos[0], node.pos[1]);
        if (task) {
          if (node.task_id === -1) {
            node.title = LiteGraph.makeLambdaTitle(
              node,
              canvas.graph._nodes,
              task
            );
            ...
            canvas.graph.vm.alertMsg(
              graph.vm.alert_colors.error,
              graph.vm.$t("already_task_in") +
                " " +
                graph.vm.$t("task_id") +
                " " +
                node.task_id
  ...
  LGraphCanvas.showMenuNodeOptionalInputs = function(
    ...
    var menu = new LiteGraph.ContextMenu(
      entries,
      { event: e, callback: inner_clicked, parentMenu: prev_menu, node: node },
      ref_window
    );
  ...
  LGraphCanvas.preventMvLambda = function(node) {
    var graphcanvas = LGraphCanvas.active_canvas;
    var canvas = graphcanvas.canvas;
    canvas.style.pointerEvents = "auto";
    node.pos = LiteGraph.cloneObject(node.before_pos);
    node.before_pos = null;
    delete node.before_pos;
    canvas.style.zIndex = 0;
    graphcanvas.graph.change();
  };

  LGraphCanvas.mvLambda = function(beforeTask, afterTask, node) {
    if (JSON.stringify(node.before_pos) === JSON.stringify(node.pos)) {
      return;
    }
    if (beforeTask.id === afterTask.id) {
      return;
    } else {
      var node_index = beforeTask._nodes.indexOf(node);
      if (node_index < 0) {
        return;
      }
      var graphcanvas = LGraphCanvas.active_canvas;
      ...
      dialog.className = "graphdialog";
      dialog.innerHTML =
        "<h3 class='name'></h3><hr><span class='checking'></span><br/><span class='detail'></span><br><div class='btn_both'><button class='no'></button><button class='yes'></button></div>";

      var title = dialog.querySelector(".name");
      title.innerText = node.title;

      var checking = dialog.querySelector(".checking");
      checking.innerText = node.title + " " + graph.vm.$t("q_want_move");
      var detail = dialog.querySelector(".detail");
      detail.innerText =
        graph.vm.$t("detail") +
        ": " +
        beforeTask.title +
      ...
      var yes_btn = dialog.querySelector(".yes");
      yes_btn.innerText = graph.vm.$t("ok");
      yes_btn.addEventListener("click", (e) => {
        canvas.parentNode.removeChild(dialog);
        canvas.style.pointerEvents = "auto";
        if (node.task_id != -1) {
          node.task_id = -1;
        }
        ...
        if (node_index >= 0) {
          beforeTask._nodes.splice(node_index, 1);
        }

        var existed = afterTask._nodes.indexOf(node);
        if (existed < 0) {
          afterTask._nodes.push(node);
          node.task_id = afterTask.id;
          afterTask.taskComputeSize(node);
        }
        if (node.inputs) {
      ...
      var no_btn = dialog.querySelector(".no");
      no_btn.innerHTML = graph.vm.$t("cancel");
      no_btn.addEventListener("click", (e) => {
        canvas.parentNode.removeChild(dialog);
        canvas.style.pointerEvents = "auto";
        this.preventMvLambda(node);
      });
  ...
  LGraphCanvas.checkInTask = function(task, node) {
    var dialog = document.createElement("div");
    dialog.className = "graphdialog";
    dialog.innerHTML =
      "<h3 class='name'></h3><hr><span class='checking'></span><br><div class='btn_both'><button class='no'>No</button><button class='yes'>Yes</button></div>";

    var title = dialog.querySelector(".name");
    title.innerText = node.title;

    var content = dialog.querySelector(".checking");
    content.innerText = "Do you want to join the " + task.title + "?";
  ...
  LGraphCanvas.checkOutTask = function(task, node) {
    var dialog = document.createElement("div");
    dialog.className = "graphdialog";
    dialog.innerHTML =
      "<h3 class='name'></h3><hr><span class='checking'></span><br><div class='btn_both'><button class='no'>No</button><button class='yes'>Yes</button></div>";
    ...
    var content = dialog.querySelector(".checking");
    content.innerText =
      "Are you sure you want to remove it from the " + task.title + "?";
    ...
    yes_btn.addEventListener("click", () => {
      ...
      var node_index = task._nodes.indexOf(node);
      task._nodes.splice(node_index, 1);
  ...
  LGraphCanvas.prototype.showSearchBox = function(event) {
    ...
    function refreshHelper() {
      ...
      if (that.onSearchBox) {
        ...
        if (list) for (var i = 0; i < list.length; ++i) addResult(list[i]);
      } else {
        var c = 0;
        str = str.toLowerCase();
        //extras
  ...
  LGraphCanvas.onMenuNodeMode = function(value, options, e, menu, node) {
    new LiteGraph.ContextMenu(["Always", "On Event", "On Trigger", "Never"], {
      event: e,
      callback: inner_clicked,
      parentMenu: menu,
      node: node,
  ...
  LGraphCanvas.onMenuNodeRemove = function(value, options, e, menu, node) {
    ...
    if (vm) {
      vm.openDeleteCheck(node);
    } else {
      console.warn("Not found vm model.");
    }
    // node.graph.remove(node);
    // node.setDirtyCanvas(true, true);
  };

  LGraphCanvas.onMenuTaskRemove = function(value, options, e, menu, task) {
    if (!task) throw "no node passed";

    if (task.removable === false) return;

    for (var task_node in task._nodes) {
      var node = task._nodes[task_node];
      task.graph.remove(node);
    }
    task.graph.remove(task);
    task.setDirtyCanvas(true, true);
  };
  ...
  LGraphCanvas.prototype.getCanvasMenuOptions = function(e) {
    var options = null;
    if (this.getMenuOptions) options = this.getMenuOptions();
      ...
      if (task) {
        ...
        if (vm.$store.getters["language/getLanguage"] === "en") {
          options = [
            {
              content: vm.$t("add_lambda_in") + " " + task.title,
              has_submenu: true,
              callback: LGraphCanvas.onMenuAdd,
            },
            {
              content: vm.$t("clone_task"),
              task: task,
              callback: LGraphCanvas.onTaskClone,
            },
          ...
          options = [
            {
              content: task.title + vm.$t("add_lambda_in"),
              has_submenu: true,
              callback: LGraphCanvas.onMenuAdd,
            },
            {
        ...
        options = [
          { content: vm.$t("add_task"), callback: LGraphCanvas.onTaskAdd },
          //{content:"Collapse All", callback: LGraphCanvas.onMenuCollapseAll }
        ];
      }
  ...
  LGraphCanvas.prototype.getGroupMenuOptions = function(node) {
    var o = [
      ...
      // null,
      // { content: "Remove", callback: LGraphCanvas.onMenuTaskRemove }
    ];
    ...
    if (slot) {
      ...
      if (node)
        ...
        if (group) {
          ...
          group.onSelected();
          menu_info.push(null);
          // menu_info.push({ content: "Remove Task", callback: vm.openDeleteCheck });
          menu_info.push({
            content: vm.$t("remove_task"),
            callback: vm.openDeleteCheck,
          });
          // menu_info.push(null, { content: "Edit Task", has_submenu: true, submenu: { title: "Task", extra: group, options: this.getGroupMenuOptions(group) } });
        }
      }
    }
  ...
  LGraphCanvas.prototype.preventMouseEvent = function(prevent) {
    if (this.canvas) {
      if (prevent) {
  ...
  LGraphCanvas.prototype.deselectedAll = function() {
    this.deselectAllNodes();
    this.deselectAllTasks();
  };
}
