...
export function install_LGraphNode(vm, litegraph) {
    var vm = vm;
    ...
    vm.LGraphNode = LiteGraph.LGraphNode = LGraphNode;
    ...
    LGraphNode.prototype.configure = function(info) {
        if (this.graph) {
            ...
            console.log("Graph is not exists");
        }
        ...
        for (var j in info) {
            ...
            if (j == "ui_setting") {
                if (info.ui_setting.pos) {
                    if (canvas && !LiteGraph.isInt(info.ui_setting.pos[0])) {
                        this.pos = [];
                        this.pos.push(info.ui_setting.pos[0] * canvas.width);
                        this.pos.push(info.ui_setting.pos[1] * canvas.height);
                    } else {
                        this.pos = [info.ui_setting.pos[0], info.ui_setting.pos[1]];
                    }
                }
                if (info.ui_setting.size) {
                    if (canvas && !LiteGraph.isInt(info.ui_setting.size[0])) {
                        this.size = [];
                        this.size.push(info.ui_setting.size[0] * canvas.width);
                        this.size.push(info.ui_setting.size[1] * canvas.height);
                    } else {
                        this.size = [info.ui_setting.size[0], info.ui_setting.size[1]];
                    }
                }
                if (info.ui_setting.color) {
                    this.color = info.ui_setting.color;
                }
                if (info.ui_setting.bgcolor) {
                    this.bgcolor = info.ui_setting.bgcolor;
                }
                if (info.ui_setting.flags) {
                    this.flags = LiteGraph.cloneObject(info.ui_setting.flags);
                }
                if (info.ui_setting.style) {
                    this.style = info.ui_setting.style;
                }
                if (info.ui_setting.shape) {
                    this.shape = info.ui_setting.shape;
                }
            }
    ...
    LGraphNode.prototype.serialize = function() {
        ...
        if (this.dynamicInputSlot !== null && this.dynamicInputSlot !== undefined) {
            o.dynamicInputSlot = this.dynamicInputSlot;
        }

        if (this.dynamicOutputSlot !== null && this.dynamicOutputSlot !== undefined) {
            o.dynamicOutputSlot = this.dynamicOutputSlot;
        }

        if (this.inputs) {
            // o.inputs = [];
            // for (var index = 0; index < this.inputs.length; ++index) {
            //     for (var key in this.inputs[index]) {
            //         if (key === "link") {
            //             if (!this.inputs[index][key]) {
            //                 this.inputs[index][key] = -1;
            //             }
            //         } else {
            //             // o.inputs.push(this.inputs[k]);
            //         }
            //     }
            //     o.inputs.push(this.inputs[index]);
            // }
        ...
        // console.log("Type:", this.type, "props:", typeof this.properties);
        ...
        o.ui_setting = {};
        if (this.color)
            o.ui_setting.color = this.color;
        if (this.bgcolor)
            o.ui_setting.bgcolor = this.bgcolor;
        if (this.boxcolor)
            o.ui_setting.boxcolor = this.boxcolor;
        if (this.shape)
            o.ui_setting.shape = this.shape;
        if (this.pos) {
            if (canvas) {
                o.ui_setting.pos = [];
                o.ui_setting.pos.push(this.pos[0] / canvas.width);
                o.ui_setting.pos.push(this.pos[1] / canvas.height);
            } else {
                o.ui_setting.pos = this.pos;
            }
        }
        if (this.size) {
            if (canvas) {
                o.ui_setting.size = [];
                o.ui_setting.size.push(this.size[0] / canvas.width);
                o.ui_setting.size.push(this.size[1] / canvas.height);
            } else {
                o.ui_setting.size = this.size;
            }
        }
        if (this.flags)
            o.ui_setting.flags = LiteGraph.cloneObject(this.flags);
        if (this.style)
            o.ui_setting.style = this.style;
        if (this.shape)
            o.ui_setting.shape = this.shape;
    ...
    LGraphNode.prototype.getInputData = function(slot, force_update) {
        ...
        if (slot >= this.inputs.length || this.inputs[slot].link == null || this.inputs[slot].link == -1)
            return;
    ...
    LGraphNode.prototype.isInputConnected = function(slot) {
        if (!this.inputs)
            return false;
        return (slot < this.inputs.length && this.inputs[slot].link != null && this.inputs[slot].link != -1);
    }
    ...
    LGraphNode.prototype.getInputNode = function(slot) {
        ...
        if (!input || input.link === null || input.link === -1)
            return null;
     ...
     */
    LGraphNode.prototype.addOutput = function(name, mimes, readonly, type, extra_info) {
        var o = { name: name, mimes: mimes, readonly: readonly, type: type, links: null };
        if (extra_info)
            for (var i in extra_info)
                o[i] = extra_info[i];
     ...
     */
    LGraphNode.prototype.addInput = function(name, mimes, readonly, type, extra_info) {
        var o = { name: name, mimes: mimes, readonly: readonly, type: type, link: null };
        if (extra_info)
            for (var i in extra_info)
                o[i] = extra_info[i];
    ...
    LGraphNode.prototype.connect = function(slot, target_node, target_slot) {
        ...
        if (target_node.task_id !== this.task_id)
            return null;
        ...
        if (target_node.inputs[target_slot].link != null || target_node.inputs[target_slot].link != -1)
    ...
    */
    ...
    LGraphNode.prototype.getJoinedTaskName = function (task_id) {
        if (!this.graph._tasks) {
            return ""; // No have task in graph.
        }
        for (var task of this.graph._tasks) {
            if (task.id === task_id) {
                return task.title; // Found task.
            }
        }
        return ""; // Not found task.
    }

    LGraphNode.prototype.getJoinedTask = function () {
        if (!this.graph._tasks) {
            return null; // No have task in graph.
        }
        for (var task of this.graph._tasks) {
