<template>
  ...
  >
    ...
    <!-- <div v-if="enabled" style="position: absolute; top: -35px; left: calc(50% - 12px); cursor: pointer;"> -->
    <div v-if="enabled && useOptionbutton" class="option-button-container">
      <v-card-actions class="option-button-layout">
        <div
          v-if="operator"
          class="option-button option-button--left align-center"
        >
          <v-icon
            v-if="returnSizable"
            size="19"
            @click.stop="defaultScreenPanel"
            :title="$t('defaultscreen')"
            >fullscreen_exit
          </v-icon>
          <v-icon
            v-else
            size="19"
            @click.stop="fullscreenPanel"
            :title="$t('fullscreen')"
            >fullscreen
          </v-icon>
        </div>
        <div v-if="operator" class="option-button option-button--right">
          <v-icon
            size="19"
            @click.stop="deletePanel"
            :title="$t('delete_panel')"
            >close
          </v-icon>
        </div>
      </v-card-actions>
...
<script>
/**
 * This is vue-draggable-resizable component. Answer DashboardPanel.
 * See https://github.com/mauricius/vue-draggable-resizable
 * @author hadoo 2019-06-25 - edit version.
 */
...
/**
 * DashboardPanel from vue-draggable-resizable. Edit version.
 * @author hadoo, 2019-06-26
 */
export default {
  replace: true,
  name: "adbDashboardPanel",
  props: {
    useOptionbutton: {
      type: Boolean,
      default: true,
  ...
  methods: {
    deletePanel() {
      this.$emit("deletePanel");
    },
    fullscreenPanel() {
      this.$emit("fullscreen");
    },
    defaultScreenPanel() {
      this.$emit("defaultScreen");
    },
    ...
    handleMove(e) {
      ...
      var event = { x: this.left, y: this.top, w: this.width, h: this.height };
