...
class PgInfoTestCase(PostgresqlTestCase):
    ...
    async def test_upsert_info(self):
        self.assertEqual(1, len(await self.db.get_infos()))
        ...
        self.assertEqual(2, len(await self.db.get_infos()))
        info1 = await self.db.get_info_by_key(key)
        ...
        self.assertEqual(2, len(await self.db.get_infos()))
        info2 = await self.db.get_info_by_key(key)
