...
class PgWidgetTestCase(PostgresqlTestCase):
    ...
    async def test_create_and_get(self):
        ...
        result1_uid = await self.db.insert_widget(
            self.layout_uid, name1, created_at=created_at1
        )
        result2_uid = await self.db.insert_widget(
            self.layout_uid, name2, created_at=created_at2
        )
        widget1 = await self.db.select_widget_by_name(self.layout_uid, name1)
        widget2 = await self.db.select_widget_by_name(self.layout_uid, name2)
        self.assertEqual(result1_uid, widget1.uid)
        self.assertEqual(result2_uid, widget2.uid)

        self.assertEqual(self.layout_uid, widget1.layout_uid)
        self.assertEqual(self.layout_uid, widget2.layout_uid)
    ...
    async def test_update_description(self):
        name1 = "widget1"
        name2 = "widget2"
        await self.db.insert_widget(self.layout_uid, name1)
        await self.db.insert_widget(self.layout_uid, name2)
        widget1_uid = (await self.db.select_widget_by_name(self.layout_uid, name1)).uid
        ...
        await self.db.update_widget_description_by_name(
            self.layout_uid, name2, desc2, updated_at2
        )
    ...
    async def test_update_extra(self):
        ...
        await self.db.update_widget_extra_by_name(
            self.layout_uid, name2, extra2, updated_at2
        )
    ...
    async def test_delete(self):
        ...
        widgets1 = await self.db.select_widget_by_layout_uid(self.layout_uid)
        self.assertEqual(2, len(widgets1))

        await self.db.delete_widget_by_uid(widget1_uid)
        await self.db.delete_widget_by_name(self.layout_uid, name2)

        widgets2 = await self.db.select_widget_by_layout_uid(self.layout_uid)
