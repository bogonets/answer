...
class ContextPermission(ContextBase):
    ...
    async def update_permission_description_by_uid(
        self, uid: int, description: str
    ) -> None:
        await self.database.update_permission_description_by_uid(uid, description)

    async def update_permission_description_by_name(
        self, name: str, description: str
    ) -> None:
        await self.database.update_permission_description_by_name(name, description)

    async def update_permission_features_by_uid(
        self, uid: int, features: List[str]
    ) -> None:
        await self.database.update_permission_features_by_uid(uid, features)

    async def update_permission_features_by_name(
        self, name: str, features: List[str]
    ) -> None:
        await self.database.update_permission_features_by_name(name, features)

    async def update_permission_extra_by_uid(self, uid: int, extra: Any) -> None:
        await self.database.update_permission_extra_by_uid(uid, extra)

    async def update_permission_extra_by_name(self, name: str, extra: Any) -> None:
        await self.database.update_permission_extra_by_name(name, extra)
    ...
    async def delete_permission_by_name(self, name: str) -> None:
        await self.database.delete_permission_by_name(name)

    async def get_permission_uid_by_name(self, name: str) -> int:
        return await self.database.get_permission_uid_by_name(name)

    async def get_permission_by_uid(
        self, uid: int, remove_sensitive=True
    ) -> Permission:
    ...
    async def get_permission_by_name(
        self, name: str, remove_sensitive=True
    ) -> Permission:
        result = await self.database.get_permission_by_name(name)
        if remove_sensitive:
            result.remove_sensitive()
        return result
