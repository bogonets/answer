...
def normalize_config_key(key: str, remove_prefix: Optional[str] = None) -> str:
...
def get_filtered_namespace(
    ...
    for k, v in filtered_envs.items():
        normalized_envs[normalize_config_key(k, start_prefix)] = v

    return Namespace(**normalized_envs)


def get_namespace_by_os_envs(start_prefix: Optional[str] = None) -> Namespace:
    return get_filtered_namespace(get_os_envs_dict(), start_prefix)
